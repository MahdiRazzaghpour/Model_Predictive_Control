...
MAX_ACCEL = 4                # maximum accel [m/ss]
...
Q = np.diag([1, 1, 10000, 1]) # state cost matrix  x = [x, y, v, yaw]
...
def calc_error(vehicle,neighbor):
    ...
    value = delta_d - inter_vehicle_distance
    return value
# ==============================================================================
    ...
    def linear_mpc_control(xref, xbar, vehicle, dref, vehicles_list):
        ...
        for t in range(T):
            ...
            for leader in vehicle.leaders:
                ...
                #     cost += distance_weight**2 * (z - (inter_vehicle_distance*(vehicle.ID-leader.ID)))
                ...
                if (vehicle.ID - leader.ID) == 1 :
                    for front in vehicles_list:
                        if front.ID == leader.ID:
                            ...
                            cost += distance_weight**2 * (z - inter_vehicle_distance)
...
def main():
    ...
    #     diff = 0
    #     while diff <= inter_vehicle_distance:
    ...
    for i in range(Number_of_vehicels):
        ...
        #print(initial_x)
        print(i,(Number_of_vehicels-i-1)*inter_vehicle_distance)
        vehicle = Agent(i, (Number_of_vehicels-i-1)*inter_vehicle_distance , 0, [cx[-1],cy[-1]] )
